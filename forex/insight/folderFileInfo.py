...
def numOfFiles(foldername, parentfolder):
    num = 0
    listFiles = os.listdir(parentfolder + '/forex/data/test_data/' + foldername + '/web')
    for file in listFiles:
        if file != 'links.txt':
            if os.path.getsize(parentfolder + "/forex/data/test_data/" + foldername + '/web/' + file) > 0:
                num += 1
    return num
...
def getNonEmptyFiles(foldername, parentfolder):
    ...
    for file in listFiles:
        if file != 'links.txt':
            if os.path.getsize(parentfolder + "/forex/data/test_data/" + foldername + '/web/' + file) > 0:
                # num+=1
                nonEmpty.append(parentfolder + "/forex/data/test_data/" + foldername + '/web/' + file)
    return nonEmpty
